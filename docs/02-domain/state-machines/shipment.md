...
| `CONFIRMED` | Retailer confirmed receipt with photo proof |
...
| STM-SHP-T06 | DELIVERED | CONFIRMED | retailer | POST /api/v1/orders/{id}/confirm-delivery | BR-SHP-060 | BR-SHP-060, BR-ORD-070 |
...
| STM-SHP-T12 | (terminal) | (terminal) | — | CONFIRMED, CANCELLED, REJECTED, DISPUTED, ESCALATED are end states; no further transitions allowed | — | — |

`STM-SHP-T05`, `T07`, `T09`, `T10`, `T11` accept multi-value `from-state` — the transition is valid from any of the listed source states.

`STM-SHP-T12` is a sentinel row stating that the listed terminal states accept no outgoing transitions. Code path: `ShipmentService.validateTransition()` `case CONFIRMED, CANCELLED, REJECTED, DISPUTED, ESCALATED -> false`.
