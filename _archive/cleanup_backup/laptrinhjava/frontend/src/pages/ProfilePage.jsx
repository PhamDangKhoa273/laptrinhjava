...
export function ProfilePage() {
  ...
  const assignedRoles = useMemo(() => Array.isArray(user?.roles) ? user.roles.map((item) => item.name || item) : [], [user])

  const roleSummary = useMemo(() => {
    if (assignedRoles.length > 1) {
      return 'Tài khoản này đang được gán nhiều vai trò cùng lúc. Đây là dữ liệu demo chưa sạch. Flow Phase 0 chuẩn là self-register Guest, Farm, Retailer; còn Driver, Shipping Manager và Admin phải được cấp nội bộ.'
    }
    ...
    return 'Tài khoản Guest dùng cho trải nghiệm công khai của BICAP. Nếu cần Farm hoặc Retailer, nên đăng ký đúng actor ngay từ đầu để tránh lệch dashboard.'
    ...
    <section className="page-section">
      ...
      <div className="profile-summary-grid">
        ...
        <article className="glass-card">
          ...
          <strong>{assignedRoles.length ? assignedRoles.join(', ') : role}</strong>
        </article>
        <article className="glass-card">
      ...
      {assignedRoles.length > 1 ? (
        <div className="alert alert-warning">User này đang có nhiều role cùng lúc ({assignedRoles.join(', ')}), nên UI có thể ưu tiên một role làm role chính để điều hướng. Đây là dữ liệu role chưa sạch, không phải flow register chuẩn.</div>
      ) : (
        <div className="alert alert-info">Primary role hiện tại: <strong>{role}</strong>. Guest, Farm và Retailer là 3 actor self-register hợp lệ trong Phase 0.</div>
      )}
