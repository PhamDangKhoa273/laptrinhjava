...
function AnnouncementCard({ item }) {
  return (
    <article className="mp-card" style={{ cursor: 'default' }}>
      <div className="mp-card__body">
        <span className="mp-card__category">{item.category || 'ANNOUNCEMENT'}</span>
        <h3 className="mp-card__title">{item.title}</h3>
        {item.summary ? <p className="mp-card__desc">{item.summary}</p> : null}
        {item.body ? <div className="mp-card__desc" dangerouslySetInnerHTML={{ __html: sanitizeAnnouncementHtml(item.body) }} /> : null}
        <div className="mp-card__meta">
          <span className="mp-card__batch">Publish: {item.publishAt ? formatDate(item.publishAt) : 'N/A'}</span>
          {item.expireAt ? <span className="mp-card__batch">Expire: {formatDate(item.expireAt)}</span> : null}
          {item.pinned ? <span className="mp-card__batch">?? Pinned</span> : null}
        </div>
      </div>
    </article>
  )
...
export function GuestMarketplacePage() {
  ...
  const initialHarvestFrom = searchParams.get('harvestFrom') || ''
  const initialHarvestTo = searchParams.get('harvestTo') || ''
  ...
  const [harvestFrom, setHarvestFrom] = useState(initialHarvestFrom)
  const [harvestTo, setHarvestTo] = useState(initialHarvestTo)
  ...
  const hasActiveFilters = Boolean(search || province || minPrice || maxPrice || certification || type || availableOnly || verifiedOnly || harvestFrom || harvestTo || sort !== 'createdAt,desc')
  ...
  useEffect(() => {
    ...
    if (availableOnly) nextParams.set('availableOnly', 'true')
    if (verifiedOnly) nextParams.set('verifiedOnly', 'true')
    if (harvestFrom) nextParams.set('harvestFrom', harvestFrom)
    if (harvestTo) nextParams.set('harvestTo', harvestTo)
  ...
  useEffect(() => {
    ...
    async function loadAnnouncements() {
      try {
        ...
        const data = await getPublicAnnouncementFeed()
      ...
      <div className="mp-hero">
        ...
        <form className="mp-hero__search" onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="mp-hero__filters-grid">
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}><input type="checkbox" checked={availableOnly} onChange={(event) => { setAvailableOnly(event.target.checked); setPage(0) }} /> Ch? hi?n c?n h?ng</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}><input type="checkbox" checked={verifiedOnly} onChange={(event) => { setVerifiedOnly(event.target.checked); setPage(0) }} /> Ch? ch?ng ch? ?? verify</label>
            <input className="mp-hero__search-input" type="date" value={harvestFrom} onChange={(event) => { setHarvestFrom(event.target.value); setPage(0) }} />
            <input className="mp-hero__search-input" type="date" value={harvestTo} onChange={(event) => { setHarvestTo(event.target.value); setPage(0) }} />
      ...
      <section style={{ marginBottom: 28 }}>
        <div className="section-heading" style={{ marginBottom: 16 }}>
          <div>
            ...
            <h2 style={{ marginBottom: 6 }}>Platform updates, events, educational announcements</h2>
            <p style={{ margin: 0, color: '#5f6b7a' }}>Thông báo công khai cho guest xem trực tiếp.</p>
          </div>
        </div>
        {announcementLoading ? (
          <div className="mp-grid">
            {Array.from({ length: 3 }).map((_, index) => <ProductCardSkeleton key={`announcement-${index}`} />)}
          </div>
        ) : announcementItems.length === 0 ? (
          <div className="mp-empty">
            <span className="mp-empty__icon">🔔</span>
            <h3>Chưa có thông báo công khai</h3>
            <p>Admin có thể đẩy platform update, event, hoặc educational announcement.</p>
          </div>
        ) : (
          <div className="mp-grid">
            {announcementItems.map((item) => (
              <article key={item.announcementId} className="mp-card" style={{ cursor: 'default' }}>
                <div className="mp-card__body">
                  <span className="mp-card__category">{item.category || 'ANNOUNCEMENT'}</span>
                  <h3 className="mp-card__title">{item.title}</h3>
                  {item.summary ? <p className="mp-card__desc">{item.summary}</p> : null}
        ...
        <div className="section-heading" style={{ marginBottom: 16, marginTop: 24 }}>
          <div>
