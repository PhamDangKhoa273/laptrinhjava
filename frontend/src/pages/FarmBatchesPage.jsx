...
function statusLabel(value) {
  ...
  return labels[String(value || '').toUpperCase()] || value || 'Mới tạo'
}

function Status({ value, expired }) {
  if (expired) return <span className="ship-status red">Hết hạn</span>
...
function isExpired(batch) {
  return Boolean(batch?.expiryDate && String(batch.expiryDate).slice(0, 10) < todayIso())
}
...
export function FarmBatchesPage() {
  ...
  const stats = useMemo(() => ({
    ...
    ready: batches.filter((b) => b.batchStatus === 'READY' && !isExpired(b)).length,
  ...
  async function handleGenerateQr(batchId) {
    ...
    try {
      ...
      setQrResult(qr)
      setSuccess('Đã sinh QR cho lô hàng.')
    } catch (err) {
  ...
  return (
    <section className="shipping-prototype-shell">
      <PageChrome
        ...
        subtitle={`${stats.total} lô • ${stats.ready} sẵn sàng • ${stats.expired} hết hạn • ${stats.available.toLocaleString('vi-VN')} kg còn lại`}
        loading={loading}
        error={error}
        success={success}
      ...
      >
        <section className="ship-metrics four">
          ...
          <Metric icon="event_busy" label="Hết hạn" value={stats.expired} tone="red" />
            ...
            {seasons.length === 0 ? (
              ...
              <div className="ship-table-wrap">
                <table className="ship-table">
                  <thead>
                    <tr><th>Mã lô</th><th>Mùa vụ</th><th>Thu hoạch</th><th>Hạn dùng</th><th>Số lượng còn/tổng</th><th>Grade</th><th>Trạng thái</th><th></th></tr>
                  </thead>
                  <tbody>
                    {batches.map((b) => (
                      <tr key={b.batchId}>
                        ...
                        <td><Status value={b.batchStatus} expired={isExpired(b)} /></td>
                        <td>
                          <div style={{ display: 'flex', gap: 4 }}>
                            <button type="button" onClick={() => fillBatch(b)} title="Sửa"><Icon>edit</Icon></button>
                            <button type="button" onClick={() => handleGenerateQr(b.batchId)} disabled={generatingQrId === String(b.batchId)} title="Tạo QR"><Icon>qr_code_2</Icon></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          <article className="ship-card">
            ...
            <form className="ship-form" onSubmit={submit}>
              ...
              <label style={{ display: 'grid', gap: 4 }}>
                ...
                <small style={{ color: 'var(--ship-muted)' }}>Dữ liệu lưu theo dạng YYYY-MM-DD.</small>
              </label>
              <label style={{ display: 'grid', gap: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--ship-muted)' }}>Hạn sử dụng <em style={{ color: 'var(--ship-danger)' }}>*</em></span>
                <input type="date" name="expiryDate" value={batchForm.expiryDate} onChange={handleChange} min={todayIso()} required />
                <small style={{ color: 'var(--ship-muted)' }}>Phải lớn hơn hoặc bằng hôm nay. Lô hết hạn không được đăng sàn.</small>
              </label>
              <label style={{ display: 'grid', gap: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                ...
                {editingBatchId ? <button type="button" onClick={resetForm}>Hủy</button> : null}
              </div>
            </form>
          </article>
        </div>
        ...
        {qrResult ? (
          <article className="ship-card" style={{ marginTop: 24 }}>
            <div className="ship-card-head">
              ...
              <span className="success-pill">Lô #{qrResult.batchId}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 24, alignItems: 'start' }}>
              ...
              <div>
                ...
                <p><strong>Mã truy xuất:</strong> {qrResult.traceCode || 'N/A'}</p>
                <p><strong>URL công khai:</strong> {qrResult.qrUrl ? <a href={qrResult.qrUrl} target="_blank" rel="noreferrer">{qrResult.qrUrl}</a> : 'N/A'}</p>
