...
import { searchListings, getCategories, getFilterOptions } from '../services/searchService'
...
export function PublicMarketplacePage() {
  ...
  const [filterOpts, setFilterOpts] = useState({ provinces: [], certifications: [] })
  ...
  function handleReset() {
    setSearch(''); setSort('createdAt,desc'); setPage(0)
    setCategoryFilter(''); setProvinceFilter(''); setCertFilter(''); setAvailableOnly(false)
  }
  ...
  return (
    <PublicShell>
      <header className="proto-hero">
        <div className="proto-hero-content">
          ...
          <div className="proto-chip-row">
            <button className={`proto-chip ${showFilters ? 'active' : ''}`} onClick={() => setShowFilters(!showFilters)}><span className="material-symbols-outlined" style={{fontSize:16}}>filter_list</span> Bộ lọc</button>
            {categoryFilter ? <button className="proto-chip active" onClick={() => { setCategoryFilter(''); setPage(0) }}>Loại: {categoryFilter} <span className="material-symbols-outlined" style={{fontSize:14}}>close</span></button> : null}
            {provinceFilter ? <button className="proto-chip active" onClick={() => { setProvinceFilter(''); setPage(0) }}>Nguồn gốc: {provinceFilter} <span className="material-symbols-outlined" style={{fontSize:14}}>close</span></button> : null}
            {certFilter ? <button className="proto-chip active" onClick={() => { setCertFilter(''); setPage(0) }}>Chứng nhận: {certFilter} <span className="material-symbols-outlined" style={{fontSize:14}}>close</span></button> : null}
      ...
      {showFilters ? <div style={{maxWidth:960,margin:'0 auto',padding:'20px 24px',display:'flex',flexWrap:'wrap',gap:16,alignItems:'end',background:'var(--proto-surface-variant)',borderRadius:12,marginTop:-12}}>
        <div><div className="proto-filter-sub" style={{marginBottom:6}}>Loại sản phẩm</div><select value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setPage(0) }} style={{padding:'8px 12px',borderRadius:8,border:'1px solid var(--proto-outline-variant)',background:'#fff',minWidth:160}}><option value="">Tất cả</option>{categories.map(cat => <option key={cat.categoryId || cat.slug} value={cat.categoryName}>{cat.categoryName}</option>)}</select></div>
        <div><div className="proto-filter-sub" style={{marginBottom:6}}>Nguồn gốc</div><select value={provinceFilter} onChange={e => { setProvinceFilter(e.target.value); setPage(0) }} style={{padding:'8px 12px',borderRadius:8,border:'1px solid var(--proto-outline-variant)',background:'#fff',minWidth:160}}><option value="">Tất cả</option>{filterOpts.provinces.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
        <div><div className="proto-filter-sub" style={{marginBottom:6}}>Chứng nhận</div><select value={certFilter} onChange={e => { setCertFilter(e.target.value); setPage(0) }} style={{padding:'8px 12px',borderRadius:8,border:'1px solid var(--proto-outline-variant)',background:'#fff',minWidth:160}}><option value="">Tất cả</option>{filterOpts.certifications.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}><input type="checkbox" id="available-filter" checked={availableOnly} onChange={e => { setAvailableOnly(e.target.checked); setPage(0) }} /><label htmlFor="available-filter" style={{fontSize:13,fontWeight:600,whiteSpace:'nowrap',cursor:'pointer'}}>Chỉ sản phẩm có sẵn</label></div>
      </div> : null}
