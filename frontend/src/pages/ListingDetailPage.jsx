...
export function ListingDetailPage() {
      ...
      try {
        ...
        if (traceCode || batchId) {
          try {
            ...
            if (mounted) setTraceError(getErrorMessage(err, 'Chưa tải được nhật ký truy xuất cho listing này.'))
          }
        }
      } catch (err) {
        if (mounted) {
  ...
  if (loading) {
    return <PublicShell><main className="proto-page"><PublicState loading title="Đang tải sản phẩm" /></main></PublicShell>
  }
  if (error) {
    return <PublicShell><main className="proto-page"><PublicState title="Không thể tải sản phẩm" message={error} action={<Link className="proto-btn-primary" to="/marketplace">Quay lại chợ nông sản</Link>} /></main></PublicShell>
  }
  ...
  const qrBase64 = traceData?.qrInfo?.qrImageBase64
      ...
      <main className="detail-page">
        <section className="detail-stack">
          ...
          <article className="detail-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'center' }}>
              <h2 className="auth-title">Nhật ký truy xuất công khai</h2>
              <Link className="auth-link" to={traceHref}><span className="material-symbols-outlined">qr_code</span> Xem truy xuất</Link>
            </div>
        ...
        <aside className="detail-sticky">
          <article className="detail-card">
            ...
            <div className="proto-card" style={{ display: 'grid', gap: 12, margin: '20px 0', padding: 18 }}>
              <div><small>Nông trại</small><strong>{listing.farmName || traceData?.seasonInfo?.farmName || 'Đang cập nhật'}</strong></div>
              <div><small>Khu vực</small><strong>{listing.province || traceData?.seasonInfo?.province || 'Đang cập nhật'}</strong></div>
              <div><small>Chứng nhận</small><strong>{listing.certificationStatus || 'Đang cập nhật'}</strong></div>
            </div>
            {qrBase64 ? (
              <div className="listing-qr-box">
                <img alt="QR truy xuất sản phẩm" src={`data:image/png;base64,${qrBase64}`} />
                <small>Quét QR để mở truy xuất công khai</small>
              </div>
            ) : null}
            <div className="auth-grid">
              <Link className="auth-submit-button" to={orderHref}><span className="material-symbols-outlined">login</span> Đăng nhập để đặt hàng</Link>
              <Link className="proto-btn-secondary" to={traceHref}><span className="material-symbols-outlined">qr_code</span> Truy xuất QR</Link>
            </div>
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--proto-outline-variant)' }}>
              <p><span className="material-symbols-outlined">gpp_good</span> Thông tin công khai đã được BICAP ghi nhận</p>
              <p><span className="material-symbols-outlined">visibility</span> Khách có thể xem nguồn gốc trước khi đăng nhập</p>
            </div>
          </article>
        </aside>
      </main>
