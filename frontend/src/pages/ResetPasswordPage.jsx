  ...
  return (
    <div className="auth-card">
      <div className="card-header auth-form-header">
        ...
        <p>Your new password must be at least 8 characters long and include a mix of uppercase, lowercase letters, and numbers.</p>
      </div>
      ...
      {message ? (
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem' }}>
            ...
            />
            <label htmlFor="show-pass" style={{ cursor: 'pointer' }}>Show password</label>
          </div>

          {error ? <div className="alert alert-error">{error}</div> : null}

          <Button type="submit" disabled={!canSubmit || loading || !token}>
            {loading ? 'Updating password...' : 'Reset password'}
          </Button>

          <p className="muted-text">
            Wait, I remember it! <Link to="/login">Back to Sign in</Link>
          </p>
        </form>
      )}
    </div>
