...
function OverviewPage(p) { const deliveredCount = p.displayShipments.filter((s)=>String(s.status).toUpperCase()==='DELIVERED').length; const delayedCount = p.displayShipments.filter((s)=>['REJECTED','DISPUTED','ESCALATED'].includes(String(s.status).toUpperCase())).length; const statuses = ['ASSIGNED','PICKED_UP','IN_TRANSIT','DELIVERED','CANCELLED']; return <PageChrome eyebrow="Quản lý vận chuyển" title="Tổng quan vận chuyển" subtitle="Điều phối đơn hàng, đội xe, tài xế và cảnh báo vận hành." loading={p.loading} error={p.error} success={p.success} actions={<button onClick={p.loadWorkspace}><Icon>refresh</Icon>Làm mới</button>}><section className="ship-metrics"><Metric icon="inventory" label="Chờ phân bổ" value={p.eligibleOrders.length} note="Đơn đủ điều kiện" tone="blue"/><Metric icon="local_shipping" label="Đang vận chuyển" value={p.displayShipments.length} note="Chuyến hàng" tone="amber"/><Metric icon="task_alt" label="Đã giao" value={deliveredCount} note="Đã giao"/><Metric icon="warning" label="Cần xử lý" value={delayedCount} note="Trạng thái ngoại lệ" tone="red"/><Metric icon="badge" label="Tài xế hoạt động" value={p.activeDrivers.length} note="Tài xế đang trực"/></section><div className="ship-overview-grid"><ShipmentStatusChart shipments={p.displayShipments} /><aside className="ship-stack"><AlertsCard reports={p.shipmentReports} /><ActivityCard shipments={p.displayShipments} /></aside></div><PendingOrders orders={p.eligibleOrders} setShipmentForm={p.setShipmentForm} /></PageChrome> }
...
function MapPanel({ shipments = [] }) { const visible = shipments.slice(0, 2); return <div className="ship-map">{visible.map((s, index)=><div className={`map-marker ${index===0?'one':'two'}`} key={s.shipmentId}><Icon>local_shipping</Icon><b>{s.vehiclePlateNo || `Chuyến #${s.shipmentId}`}</b><small>{s.status || 'CHƯA XÁC ĐỊNH'}</small></div>)}{visible.length===0?<div className="map-marker one"><Icon>map</Icon><b>Chưa có chuyến hàng</b><small>Backend chưa trả về vị trí vận chuyển.</small></div>:null}<div className="map-controls"><button type="button">+</button><button type="button">−</button></div></div> }
function AlertsCard({ reports = [] }) { const openReports = reports.filter((r)=>String(r.status).toUpperCase()==='OPEN').slice(0,3); return <article className="ship-card"><div className="ship-card-head"><h3><Icon>emergency_home</Icon>Cảnh báo vận hành</h3><span className="danger-pill">{openReports.length} đang mở</span></div>{openReports.length ? openReports.map((r,i)=><div className={`ship-warning w${i}`} key={r.reportId}><strong>{r.issueType || 'Báo cáo sự cố'}</strong><p>{r.description || `Chuyến #${r.shipmentId}`}</p></div>) : <p>Không có báo cáo sự cố đang mở.</p>}</article> }
function ActivityCard({ shipments = [] }) { return <article className="ship-card"><h3>Hoạt động gần đây</h3><div className="ship-timeline">{shipments.slice(0,3).map((s)=><div key={s.shipmentId}><i /><strong>Chuyến #{s.shipmentId}</strong><p>{s.status || 'CHƯA XÁC ĐỊNH'} · {s.updatedAt || s.createdAt || 'Chưa có mốc thời gian'}</p></div>)}{shipments.length===0?<p>Chưa có hoạt động chuyến hàng.</p>:null}</div></article> }
function PendingOrders({ orders, setShipmentForm }) { const list = safeList(orders); return <article className="ship-card"><div className="ship-card-head"><h3>Đơn hàng chờ phân bổ</h3><a href="/shipping/orders">Tạo chuyến</a></div>{list.length ? <div className="ship-table-wrap"><table className="ship-table"><thead><tr><th>Mã đơn</th><th>Điểm xuất phát</th><th>Điểm đến</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{list.map(o=><tr key={o.orderId}><td>#{o.orderId}</td><td>{o.farmName || 'Nông trại'}</td><td>{o.retailerName || 'Nhà bán lẻ'}</td><td><Status value={o.orderStatus} /></td><td><button onClick={()=>setShipmentForm(prev=>({...prev, orderId:String(o.orderId)}))}>Chọn</button></td></tr>)}</tbody></table></div> : <p>Không có đơn đủ điều kiện tạo chuyến.</p>}</article> }
...
function ShipmentTable({ shipments }) { if (!shipments.length) return <p>Chưa có chuyến hàng từ backend.</p>; return <div className="ship-table-wrap"><table className="ship-table large"><thead><tr><th>Mã chuyến</th><th>Điểm xuất</th><th>Điểm đến</th><th>Tài xế & xe</th><th>Trạng thái</th><th>Cập nhật</th><th>Thao tác</th></tr></thead><tbody>{shipments.map((s)=><tr key={s.shipmentId}><td><b>#{s.shipmentId}</b><small>Đơn #{s.orderId || 'N/A'}</small></td><td>{s.farmName || 'Nông trại'}</td><td>{s.retailerName || 'Nhà bán lẻ'}</td><td>{s.driverCode ? `${s.driverCode} • ${s.vehiclePlateNo || 'N/A'}` : <button className="dash-btn">Gán tài xế</button>}</td><td><Status value={s.status} /></td><td>{s.updatedAt || s.createdAt || 'N/A'}</td><td><button><Icon>more_vert</Icon></button></td></tr>)}</tbody></table></div> }
function CreateShipmentPage(p) { return <PageChrome eyebrow="Vận chuyển / Đơn hàng / Chuyến mới" title="Lập lệnh vận chuyển" subtitle="Cấu hình logistics và xác thực blockchain cho đơn hàng nông sản." loading={p.loading} error={p.error} success={p.success}><form className="ship-create-grid" onSubmit={p.submitShipment}><div className="ship-form-stack"><FormSection step="1" title="Định danh đơn hàng"><label>Đơn hàng liên quan<select name="orderId" value={p.shipmentForm.orderId} onChange={p.onShipmentChange} required><option value="">Chọn đơn hàng</option>{safeList(p.eligibleOrders).map(o=><option key={o.orderId} value={o.orderId}>#{o.orderId} • {o.farmName} → {o.retailerName}</option>)}</select></label></FormSection><FormSection step="2" title="Điểm giao nhận"><div className="node-card"><Icon>location_on</Icon><input value={p.eligibleOrders.find(o=>String(o.orderId)===String(p.shipmentForm.orderId))?.farmName || ''} readOnly placeholder="Nơi lấy hàng sẽ tự xác định theo đơn" /></div><div className="node-line"/><div className="node-card"><Icon>local_shipping</Icon><input value={p.eligibleOrders.find(o=>String(o.orderId)===String(p.shipmentForm.orderId))?.retailerName || ''} readOnly placeholder="Nơi giao hàng sẽ tự xác định theo đơn" /></div></FormSection><FormSection step="3" title="Phân bổ tài nguyên"><div className="two-cols"><label>Tài xế<select name="driverId" value={p.shipmentForm.driverId} onChange={p.onShipmentChange}><option value="">Chọn tài xế</option>{p.activeDrivers.map(d=><option key={d.driverId} value={d.driverId}>{d.driverCode} • {d.userFullName}</option>)}</select></label><label>Phương tiện<select name="vehicleId" value={p.shipmentForm.vehicleId} onChange={p.onShipmentChange}><option value="">Chọn phương tiện</option>{p.activeVehicles.map(v=><option key={v.vehicleId} value={v.vehicleId}>{v.plateNo} • {v.vehicleType}</option>)}</select></label></div></FormSection><FormSection title="Ghi chú đặc biệt"><textarea name="note" value={p.shipmentForm.note} onChange={p.onShipmentChange} placeholder="Yêu cầu xử lý, ngưỡng nhiệt độ, ghi chú cho sổ cái..." /></FormSection></div><aside className="ship-summary"><MapPanel shipments={p.displayShipments} /><div className="protocol-card"><Icon fill>verified_user</Icon><h3>Xác thực blockchain</h3><p>Bằng chứng vận chuyển sẽ được backend commit sau khi tạo và cập nhật trạng thái.</p><small>HASH sẽ được sinh ra bởi dịch vụ blockchain.</small></div><button className="primary-action" disabled={p.savingShipment}><Icon>send</Icon>{p.savingShipment?'Đang tạo...':'Xác nhận & tạo chuyến'}</button><button type="button" className="secondary-action">Xóa biểu mẫu</button></aside></form></PageChrome> }
...
function TrackingPage(p) { return <PageChrome eyebrow="Vận chuyển / Theo dõi" title="Theo dõi hành trình" subtitle="Trạng thái chuyến hàng trực tiếp từ backend." loading={p.loading} error={p.error} success={p.success}><div className="tracking-layout"><div className="tracking-map"><MapPanel shipments={p.displayShipments} /><div className="tracking-stats"><Metric icon="check_circle" label="Đã giao" value={p.displayShipments.filter((s)=>String(s.status).toUpperCase()==='DELIVERED').length} note="API"/><Metric icon="sensors" label="Đang hoạt động" value={p.displayShipments.length} note="Trực tiếp" tone="blue"/></div></div><aside className="tracking-list"><h3>Lô hàng đang đi <span className="live-pill"><i/>Trực tiếp</span></h3>{p.displayShipments.length ? p.displayShipments.map((s,i)=><div className={`tracking-card ${i===0?'active':''}`} key={s.shipmentId}><strong>Chuyến #{s.shipmentId}</strong><p>{s.farmName} → {s.retailerName}</p><div className="progress"><i style={{width:String(s.status).toUpperCase()==='DELIVERED'?'100%':String(s.status).toUpperCase()==='IN_TRANSIT'?'68%':'35%'}} /></div><Status value={s.status}/></div>) : <p>Chưa có chuyến hàng đang hoạt động.</p>}</aside></div></PageChrome> }
function DriversPage(p) { return <PageChrome eyebrow="Vận chuyển / Tài xế" title="Quản lý tài xế" subtitle={`${p.displayDrivers.length} tài xế trong mạng lưới • ${p.activeDrivers.length} đang hoạt động`} loading={p.loading} error={p.error} success={p.success}><section className="ship-metrics four"><Metric icon="group" label="Tổng số tài xế" value={p.displayDrivers.length} note="API"/><Metric icon="radio_button_checked" label="Đang hoạt động" value={p.activeDrivers.length} note="Kích hoạt"/><Metric icon="star" label="Đã gán chuyến" value={p.displayShipments.filter((s)=>s.driverCode).length} note="Phân bổ" tone="amber"/><Metric icon="task_alt" label="Đã giao" value={p.displayShipments.filter((s)=>String(s.status).toUpperCase()==='DELIVERED').length} note="Đã giao" tone="blue"/></section><div className="drivers-grid"><article className="ship-card"><DriverTable drivers={p.displayDrivers} fillDriver={p.fillDriver} handleDeleteDriver={p.handleDeleteDriver}/></article><DriverForm {...p}/></div><article className="ship-card"><h3>Bản đồ tài xế thời gian thực</h3><MapPanel shipments={p.displayShipments} /></article></PageChrome> }
function DriverTable({ drivers, fillDriver, handleDeleteDriver }) { if (!drivers.length) return <p>Chưa có tài xế từ backend.</p>; return <div className="ship-table-wrap"><table className="ship-table"><thead><tr><th>Tài xế</th><th>Liên hệ</th><th>Trạng thái</th><th>Đơn hiện tại</th><th>Đánh giá</th><th></th></tr></thead><tbody>{drivers.map(d=><tr key={d.driverId}><td><b>{d.userFullName || d.driverCode}</b><small>ID: {d.driverId}</small></td><td>{d.phone || 'N/A'}<small>{d.email || d.driverCode || ''}</small></td><td><Status value={d.status}/></td><td><div className="progress"><i style={{width:d.status==='ACTIVE'?'75%':'30%'}}/></div></td><td>{d.rating || 'N/A'}</td><td style={{display:'flex',gap:4}}><button onClick={()=>fillDriver(d)}><Icon>edit</Icon></button><button onClick={()=>handleDeleteDriver(d.driverId)} style={{color:'var(--proto-error)'}}><Icon>delete</Icon></button></td></tr>)}</tbody></table></div> }
...
function VehiclesPage(p) { return <PageChrome eyebrow="Vận chuyển / Phương tiện" title="Quản lý phương tiện" subtitle="Biển số, tải trọng, trạng thái vận hành." loading={p.loading} error={p.error} success={p.success}><div className="drivers-grid"><article className="ship-card"><div className="ship-table-wrap"><table className="ship-table"><thead><tr><th>Biển số</th><th>Loại xe</th><th>Tải trọng</th><th>Trạng thái</th><th></th></tr></thead><tbody>{p.displayVehicles.map(v=><tr key={v.vehicleId}><td><b>{v.plateNo}</b></td><td>{v.vehicleType}</td><td>{formatCapacity(v.capacity)}</td><td><Status value={v.status}/></td><td style={{display:'flex',gap:4}}><button onClick={()=>p.fillVehicle(v)}><Icon>edit</Icon></button><button onClick={()=>p.handleDeleteVehicle(v.vehicleId)} style={{color:'var(--proto-error)'}}><Icon>delete</Icon></button></td></tr>)}{p.displayVehicles.length===0?<tr><td colSpan="5">Chưa có phương tiện từ backend.</td></tr>:null}</tbody></table></div></article><article className="ship-card"><h3>{p.selectedVehicleId?'Cập nhật phương tiện':'Thêm phương tiện'}</h3><form className="ship-form" onSubmit={p.submitVehicle}><input name="plateNo" placeholder="Biển số" value={p.vehicleForm.plateNo} onChange={p.onVehicleChange} disabled={Boolean(p.selectedVehicleId)} required/><input name="vehicleType" placeholder="Loại xe" value={p.vehicleForm.vehicleType} onChange={p.onVehicleChange} required/><input name="capacity" type="number" placeholder="Tải trọng (kg)" value={p.vehicleForm.capacity} onChange={p.onVehicleChange} required/><input name="status" value={p.vehicleForm.status} onChange={p.onVehicleChange}/><button disabled={p.savingVehicle}>Lưu phương tiện</button></form></article></div></PageChrome> }
function NotificationsPage(p) { return <PageChrome eyebrow="Vận chuyển / Thông báo" title="Thông báo logistics" subtitle="Nhận/gửi thông tin thay đổi chuyến hàng." loading={p.loading} error={p.error} success={p.success}><article className="ship-card">{p.notifications.length===0?<p>Chưa có thông báo.</p>:p.notifications.map(n=><div className="notification-row" key={n.notificationId}><div><strong>{n.title}</strong><p>{n.message}</p></div>{!n.read?<button onClick={()=>p.handleMarkRead(n.notificationId)}>Đánh dấu đọc</button>:<Status value="ĐÃ ĐỌC"/>}</div>)}</article></PageChrome> }
function ReportsPage(p) { const reports = safeList(p.shipmentReports); return <PageChrome eyebrow="Vận chuyển / Báo cáo" title="Báo cáo vận hành" subtitle="Sự cố tài xế, bằng chứng, quét QR và SLA." loading={p.loading} error={p.error} success={p.success} actions={<button onClick={p.loadWorkspace}><Icon>refresh</Icon>Làm mới</button>}><VerificationHub shipments={p.displayShipments} /><article className="ship-card"><div className="ship-card-head"><h3>Hộp thư sự cố tài xế</h3><span className="danger-pill">{reports.filter((r)=>String(r.status).toUpperCase()==='OPEN').length} đang mở</span></div>{reports.length ? <div className="ship-table-wrap"><table className="ship-table"><thead><tr><th>Báo cáo</th><th>Chuyến</th><th>Tài xế</th><th>Sự cố</th><th>Mức độ</th><th>Trạng thái</th></tr></thead><tbody>{reports.map((r)=><tr key={r.reportId}><td><b>#{r.reportId}</b><small>{r.description || 'Không có mô tả'}</small></td><td>#{r.shipmentId}</td><td>{r.driverId || 'N/A'}</td><td>{r.issueType}</td><td><Status value={r.severity}/></td><td><Status value={r.status}/></td></tr>)}</tbody></table></div> : <p>Chưa có báo cáo sự cố từ tài xế.</p>}</article></PageChrome> }
function VerificationHub({ shipments = [] }) { const latest = shipments[0]; return <section className="verification-grid"><article className="ship-card"><h3><Icon fill>verified_user</Icon>Trung tâm xác thực</h3><div className="ship-timeline"><div><i/><strong>Chuyến đang chọn</strong><p>{latest ? `Chuyến #${latest.shipmentId} · ${latest.status}` : 'Chưa chọn chuyến nào'}</p></div><div><i/><strong>Đơn hàng liên quan</strong><p>{latest?.orderId ? `Đơn #${latest.orderId}` : 'N/A'}</p></div><div><i/><strong>Bằng chứng truy xuất</strong><p>{latest?.blockchainTxHash || latest?.txHash || 'Đang chờ bằng chứng blockchain'}</p></div></div></article><article className="ship-card image-card"><span>ĐIỀU HÀNH TRỰC TIẾP</span><h3>Bảng điều khiển đội xe</h3><p>Giám sát đội xe bằng dữ liệu chuyến hàng, báo cáo và thông báo từ backend.</p></article></section> }
function SendReportPage(p) { return <PageChrome eyebrow="Vận chuyển / Báo cáo / Báo cáo mới" title="Gửi báo cáo đến Admin" subtitle="Báo cáo sự cố vận hành, đề xuất cải tiến hoặc yêu cầu hỗ trợ." loading={p.loading} error={p.error} success={p.success}><form className="ship-create-grid" onSubmit={p.submitReport}><div className="ship-form-stack"><article className="ship-card form-section"><h3><Icon>description</Icon>Nội dung báo cáo</h3><label>Tiêu đề<input name="title" value={p.reportForm.title} onChange={p.onReportChange} placeholder="VD: Xe hỏng trên đường giao hàng" required/></label><label>Mô tả chi tiết<textarea name="description" value={p.reportForm.description} onChange={p.onReportChange} rows={4} placeholder="Mô tả vấn đề, thời gian, địa điểm..." required/></label><label>Mức độ<select name="severity" value={p.reportForm.severity} onChange={p.onReportChange}><option value="LOW">Thấp</option><option value="MEDIUM">Trung bình</option><option value="HIGH">Cao</option><option value="CRITICAL">Nghiêm trọng</option></select></label><button disabled={p.savingReport}>{p.savingReport?'Đang gửi...':'Gửi báo cáo'}</button></article></div></form></PageChrome> }
function SendNotificationPage(p) { return <PageChrome eyebrow="Vận chuyển / Thông báo / Mới" title="Gửi thông báo" subtitle="Gửi thông báo đến Nông trại, Nhà bán lẻ hoặc Quản trị viên." loading={p.loading} error={p.error} success={p.success}><form className="ship-create-grid" onSubmit={p.submitNotification}><div className="ship-form-stack"><article className="ship-card form-section"><h3><Icon>notifications</Icon>Soạn thông báo</h3><label>Đối tượng<select name="recipientRole" value={p.notificationForm.recipientRole} onChange={p.onNotificationChange}><option value="FARM">Nông trại</option><option value="RETAILER">Nhà bán lẻ</option><option value="ADMIN">Quản trị viên</option></select></label><label>Tiêu đề<input name="title" value={p.notificationForm.title} onChange={p.onNotificationChange} placeholder="Tiêu đề thông báo" required/></label><label>Nội dung<textarea name="message" value={p.notificationForm.message} onChange={p.onNotificationChange} rows={4} placeholder="Nội dung thông báo..." required/></label><button disabled={p.savingNotification}>{p.savingNotification?'Đang gửi...':'Gửi thông báo'}</button></article></div></form></PageChrome> }
...
function ShippingProfilePage() {
  ...
  return <PageChrome eyebrow="Vận chuyển / Hồ sơ" title="Hồ sơ cá nhân" subtitle="Thông tin tài khoản và bảo mật.">
