...
function Status({ value }) {
  ...
  return <span className={`ship-status ${key}`}>{value || 'PENDING'}</span>
}
...
export function FarmOrdersPage() {
  ...
  async function handleReview(orderId, decision) {
    ...
    try {
      ...
      setDecisionNotes((prev) => { const next = { ...prev }; delete next[orderId]; return next })
      await load()
    } catch (err) {
      ...
      >
        ...
        <article className="ship-card">
          <div className="ship-card-head">
            <h3><Icon>inbox</Icon>Buy requests đang chờ xử lý</h3>
            <span className="danger-pill">{pendingOrders.length} PENDING</span>
          </div>
          {pendingOrders.length === 0 ? (
            ...
            <div className="ship-table-wrap">
              <table className="ship-table">
                <thead>
                  <tr><th>Order</th><th>Retailer</th><th>Items</th><th>Tổng</th><th>Tạo lúc</th><th>Quyết định</th></tr>
                </thead>
                <tbody>
                  {pendingOrders.map((o) => (
                    <tr key={o.orderId}>
                      <td><b>#{o.orderId}</b><small>{o.paymentStatus || 'UNPAID'}</small></td>
        ...
        <article className="ship-card" style={{ marginTop: 24 }}>
          ...
          {(confirmedOrders.length + completedOrders.length) === 0 ? (
            ...
            <div className="ship-table-wrap">
              <table className="ship-table">
                <thead>
                  <tr><th>Order</th><th>Retailer</th><th>Trạng thái</th><th>Thanh toán</th><th>Tổng</th><th>Cập nhật</th></tr>
                </thead>
                <tbody>
        ...
        <article className="ship-card" style={{ marginTop: 24 }}>
          ...
          {registrations.length === 0 ? (
            ...
            <div className="ship-table-wrap">
              <table className="ship-table">
                <thead>
                  <tr><th>Listing</th><th>Trạng thái</th><th>Reviewer</th><th>Ghi chú</th></tr>
                </thead>
                <tbody>
                  {registrations.map((r) => (
