...
const roleLabels = { [ROLES.FARM]: 'Nông trại', [ROLES.RETAILER]: 'Nhà bán lẻ', [ROLES.SHIPPING_MANAGER]: 'QL vận chuyển' }
...
export function RegisterPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Không thể đăng ký tài khoản. Vui lòng kiểm tra lại mật khẩu, số điện thoại hoặc email đã tồn tại.'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="auth-proto-shell auth-split">
      <aside className="auth-image-pane" style={{ backgroundImage: "linear-gradient(135deg,rgba(13,99,27,.88),rgba(0,95,175,.80)), url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=1600&q=80')" }}>
        <div className="auth-image-content">
          ...
          {[
            ['agriculture', 'Nông trại', 'Quản lý mùa vụ, lô hàng và listing nông sản.'],
            ['storefront', 'Nhà bán lẻ', 'Tìm nguồn hàng và tạo đơn mua từ marketplace.'],
            ['local_shipping', 'Quản lý vận chuyển', 'Điều phối giao hàng và theo dõi trạng thái vận đơn.'],
          ].map(([icon, title, desc]) => <div className="auth-feature" key={title}><span className="material-symbols-outlined filled" style={{ color: 'var(--proto-primary-fixed)', fontSize: 26 }}>{icon}</span><div><strong>{title}</strong><p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,.72)' }}>{desc}</p></div></div>)}
        </div>
      </aside>
      <section className="auth-form-pane">
        <div className="auth-form-card register-card">
          ...
          <form className="auth-grid" onSubmit={handleSubmit}>
            ...
            <div className="auth-conditional"><div className="proto-kicker" style={{ marginBottom: 10 }}><span className="material-symbols-outlined">{roleIcons[form.role]}</span>Không gian {roleLabels[form.role].toLowerCase()}</div><p className="public-muted" style={{ margin: 0 }}>Thông tin đăng ký được dùng để xác minh vai trò và phân quyền truy cập phù hợp.</p></div>
