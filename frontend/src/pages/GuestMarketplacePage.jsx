...
const navItems = [
  { module: 'overview', to: '/guest/overview', icon: 'dashboard', label: 'B?ng điều khi?n' },
  { module: 'search', to: '/guest/search', icon: 'search', label: 'T?m ki?m' },
  { module: 'announcements', to: '/guest/announcements', icon: 'campaign', label: 'Thông báo' },
  { module: 'education', to: '/guest/education', icon: 'school', label: 'Ki?n th?c' },
]

const PLACEHOLDER_IMG = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300" fill="%23e2e8f0"><rect width="400" height="300"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%2394a3b8" font-size="16">Nông s?n BICAP</text></svg>')
      ...
      <aside className="mp-side">
        ...
        <div className="mp-side__profile">
          ...
          <div>
            ...
            <div className="mp-side__profile-badge">Đã xác th?c blockchain</div>
          </div>
        </div>
      ...
      <div className="mp-wrap">
        <header className="mp-top">
          ...
          <form className="mp-top__search" onSubmit={submitTopSearch} style={module !== 'overview' ? { marginLeft: 0 } : undefined}>
            ...
            <input
              placeholder={module === 'search' ? 'T?m nhanh...' : 'T?m ki?m s?n ph?m, nông tr?i, mùa v?...'}
              value={topQuery}
          ...
          <div className="mp-top__actions">
            ...
            <NavLink to="/login" className="mp-icon-btn material-symbols-outlined" aria-label="Đăng nh?p">login</NavLink>
          </div>
        </header>
        {children}
      </div>
    ...
    <GuestShell module="overview">
      <main className="mp-main">
        <section className="mp-hero">
          <p className="mp-hero__eyebrow">Th? trường nông s?n</p>
          <h1 className="mp-hero__title">Truy xu?t ngu?n g?c nông s?n t? trang tr?i đến bàn ăn.</h1>
          <p className="mp-hero__subtitle">Khám phá nông s?n Vi?t được xác th?c blockchain v?i truy xu?t minh b?ch t? đất tr?ng đến k? hàng.</p>
        ...
        {loading ? (
          ...
          <div className="mp-empty">
            ...
            <h3>Chưa có s?n ph?m</h3>
            <p>Chưa có s?n ph?m nào trên th? trường.</p>
          </div>
        ) : (
          <section>
            <div className="mp-section-title">
              <div>
                ...
                <p>Các s?n ph?m đã được xác th?c và phê duy?t.</p>
              </div>
            </div>
              ...
              {listings.slice(0, 6).map((p) => (
                <article className="mp-card" key={p.listingId}>
                  <div className="mp-card__image">
                    ...
                    <span className="mp-card__badge">{p.certification || 'Đã xác th?c'}</span>
                  </div>
                  <div className="mp-card__body">
                    ...
                    <NavLink to={`/listings/${p.listingId}`} className="mp-btn-detail">Xem chi ti?t</NavLink>
                  </div>
                </article>
              ))}
            </div>
...
function SearchPage() {
  ...
  useEffect(() => {
    let mounted = true
    getFilterOptions().catch(() => ({ categories: [], provinces: [], certifications: [] })).then((opts) => {
      if (!mounted) return
  ...
  return (
    <GuestShell module="search">
      <main className="mp-main mp-main--narrow">
        <section className="mp-hero">
          <h1 className="mp-hero__title">Khám phá ch? nông s?n</h1>
          <p className="mp-hero__subtitle">Truy xu?t s?n ph?m t? đất tr?ng đến k? hàng b?ng xác th?c blockchain.</p>
          <div className="mp-hero__search">
            <div className="mp-hero__search-main">
              <input
                className="mp-hero__search-input"
                placeholder="T?m theo tên s?n ph?m, nông tr?i ho?c địa điểm..."
                value={query}
            ...
            {activeFilterCount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, opacity: 0.75 }}>
                <span>Đang l?c: {activeFilterCount} tiêu chí</span>
                <button type="button" onClick={clearFilters} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.18)', color: 'inherit', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 12 }}>Xóa b? l?c</button>
              </div>
            )}
          </div>
        </section>
        {searched && !loading && (
          <div className="mp-page-header" style={{ padding: '12px 0' }}>
            <h3 className="mp-page-header__title">
              K?t qu? t?m ki?m <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>{total} k?t qu?</span>
            </h3>
          </div>
        )}
        {loading ? (
          ...
          <div className="mp-empty">
            ...
            <h3>Không t?m th?y</h3>
            <p>Không t?m th?y k?t qu? phù h?p.</p>
          </div>
        ) : (
          <div className="mp-grid">
            {results.map((item) => (
              <article className="mp-card" key={item.listingId}>
                ...
                <div className="mp-card__body">
                  ...
                  <p className="mp-card__farm">
                    <span className="mp-card__farm-icon material-symbols-outlined">location_on</span>
                    {item.province || 'Vi?t Nam'}
                  </p>
                  <div className="mp-card__footer">
                    <span className="mp-card__price">{formatPrice(item.price)}</span>
                  </div>
                  <NavLink to={`/listings/${item.listingId}`} className="mp-btn-detail">Theo d?i lô</NavLink>
                </div>
              </article>
            ))}
          </div>
    ...
    <GuestShell module="announcements">
      <main className="mp-main mp-main--narrow">
        <div className="mp-page-header">
          <h1 className="mp-page-header__title">Thông báo m?i nh?t</h1>
          <p className="mp-page-header__desc">C?p nh?t th? trường, nâng c?p h? th?ng và thông tin mùa v? t? BICAP.</p>
        </div>
              ...
              {announcements.map((ann) => (
                <article className="mp-ann-list-item" key={ann.announcementId}
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <span className="mp-ann-card__category">{ann.category || 'C?p nh?t'}</span>
    ...
    <GuestShell module="education">
      <main className="mp-main mp-main--narrow">
        <section className="mp-hero" style={{ marginBottom: 24 }}>
          <p className="mp-hero__eyebrow">Trao quy?n minh b?ch</p>
          <h1 className="mp-hero__title">Làm ch? <b>nông nghi?p truy xu?t được</b></h1>
          <p className="mp-hero__subtitle">T?m hi?u cách blockchain b?o đảm m?i nông s?n đều có câu chuy?n t? đất tr?ng đến k? hàng.</p>
        </section>

        <section>
          <div className="mp-section-title">
            <div>
              <h2>Bài vi?t ki?n th?c</h2>
            </div>
          </div>
          {loading ? (
            <div className="mp-loading">
            ...
            <div className="mp-empty">
              ...
              <h3>Chưa có bài vi?t</h3>
              <p>Chưa có bài vi?t nào.</p>
            </div>
          ) : (
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    <div>
                      <span style={{fontSize:'0.75em',textTransform:'uppercase',letterSpacing:'1px',opacity:0.5}}>
                        {item.contentType === 'GUIDE' ? 'Hướng d?n' : item.contentType === 'VIDEO' ? 'Video' : 'Bài vi?t'}
                      </span>
