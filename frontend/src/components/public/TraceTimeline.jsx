const iconFor = (type = '') => {
  ...
  if (text.includes('plant') || text.includes('seed') || text.includes('gieo')) return 'agriculture'
  if (text.includes('care') || text.includes('irrig') || text.includes('water') || text.includes('tưới')) return 'water_drop'
  if (text.includes('harvest') || text.includes('thu hoạch')) return 'grass'
  if (text.includes('pack') || text.includes('storage') || text.includes('đóng gói')) return 'inventory_2'
  if (text.includes('ship') || text.includes('deliver') || text.includes('giao')) return 'local_shipping'
  return 'verified'
}
...
export function TraceTimeline({ items = [], fallback = false, emptyMessage = 'Chưa có nhật ký sản xuất nào được công bố cho lô hàng này.' }) {
  const fallbackItems = [
    { processType: 'Gieo trồng', description: 'Lô giống đã được ghi nhận.', recordedAt: '2023-08-01' },
  ...
  return (
    <ul className="trace-timeline">
      {data.map((item, index) => {
        ...
        const recordedAt = item.recordedAt || item.performedAt || item.createdAt || item.eventTime || item.updatedAt
        return (
          <li key={item.processId || item.id || item.stepId || `${title}-${index}`}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 24, alignItems: 'center' }}>
              ...
              <div className="proto-card" style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 12, background: 'var(--proto-surface-low)' }}>
                <span className="material-symbols-outlined" style={{ color: 'var(--proto-tertiary)' }}>{iconFor(title)}</span>
