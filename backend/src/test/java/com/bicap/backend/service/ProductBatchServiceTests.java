...
class ProductBatchServiceTests {
    ...
    void generateQrCode_shouldIncludeTraceUrlAndSerial() {
        ...
        try (org.mockito.MockedStatic<com.bicap.core.security.SecurityUtils> mocked = mockStatic(com.bicap.core.security.SecurityUtils.class)) {
            ...
            // URL format: http://localhost:5173/public/trace?traceCode=TRACE-{batchId}-{random8}
            assertTrue(response.getQrUrl().startsWith("http://localhost:5173/public/trace?traceCode=TRACE-30-"),
                    "qrUrl should embed traceCode: " + response.getQrUrl());
            assertTrue(response.getQrUrl().contains("traceCode=TRACE-30-"));
        }
    }
}
